	...
	: Object(posX, posY, posZ)	// 親クラスのコンストラクタに引数を渡す)
{
	...
	m_pModel = new Model();
	if (!m_pModel->Load("Assets/Model/Fruits/Lemon/Lemon.fbx", 1.0f, Model::Flip::XFlip))	// モデルデータ読み込み
	{
		MessageBox(NULL, "”Assets/Model/Fruits/Lemon/Lemon.fbx”が読み込めませんでした", "Error", MB_OK);	// エラーメッセージ
	}
